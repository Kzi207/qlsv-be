...
const matchesAnyTerm = (normalizedMessage, terms) => {
...
const detectUnsafeSecurityRequest = (message) => {
    ...
    if (matchesAnyTerm(normalizedMessage, promptInjectionTerms)) {
        return {
            topic: 'Bảo mật AI',
            answer: 'Mình không thể tiết lộ, thay đổi hoặc bỏ qua hướng dẫn nội bộ của hệ thống. Mình chỉ hỗ trợ các nội dung liên quan đến học tập, điểm danh, điểm rèn luyện, minh chứng, hồ sơ cá nhân và yêu cầu hỗ trợ sinh viên.',
        };
    }
    ...
    if (matchesAnyTerm(normalizedMessage, sensitiveInfoTerms)) {
        return {
            ...
            answer: 'Mình không thể cung cấp API key, token, mật khẩu, biến môi trường, mã nguồn, cookie, thông tin cơ sở dữ liệu hoặc dữ liệu cá nhân của người khác. Nếu bạn cần hỗ trợ tài khoản, hãy gửi yêu cầu hỗ trợ và chỉ cung cấp thông tin cần thiết như họ tên, mã số sinh viên, lớp và mô tả sự cố.',
        };
    }
    const exploitTerms = [
        'khai thac lo hong',
    ...
    if (matchesAnyTerm(normalizedMessage, exploitTerms)) {
        return {
            ...
            answer: 'Mình không thể hướng dẫn khai thác lỗ hổng, vượt quyền, tấn công hệ thống, giả mạo điểm danh, sửa điểm hoặc truy cập dữ liệu trái phép. Nếu bạn phát hiện lỗi bảo mật, hãy mô tả hiện tượng ở mức khái quát và gửi yêu cầu hỗ trợ để quản trị viên kiểm tra an toàn.',
        };
    }
    return null;
};
const detectOwnProfileRequest = (message) => {
    const normalizedMessage = normalizeText(message);
    const profileTerms = [
        'thong tin ca nhan',
        'ho so ca nhan',
...
const detectProfileUpdateRequest = (message) => {
    const normalizedMessage = normalizeText(message);
    const updateTerms = [
        'cap nhat thong tin',
        'sua thong tin',
...
const parseProfileUpdateInput = (message) => {
...
const formatOwnProfileAnswer = (user) => {
...
const getOwnProfileResponse = async (req) => {
    if (!req.user?.id)
        return null;
...
const updateOwnProfileFromChatbot = async (req, message) => {
    if (!req.user?.id)
        return null;
...
const detectTrainingScoreLookupRequest = (message) => {
    const normalizedMessage = normalizeText(message);
    const submissionTerms = ['nop phieu', 'gui phieu', 'tu danh gia', 'ke khai'];
    if (matchesAnyTerm(normalizedMessage, submissionTerms))
        return false;
    const lookupTerms = [
        'xem diem ren luyen',
        'xem diem drl',
...
const getOwnTrainingScoreResponse = async (req) => {
...
const detectAttendanceSummaryRequest = (message) => {
    const normalizedMessage = normalizeText(message);
    const terms = [
        'so ngay nghi',
        'so buoi nghi',
...
const getOwnAttendanceSummaryResponse = async (req) => {
    const studentId = Number(req.user?.studentId || 0);
    if (!studentId) {
        return {
...
const detectUnsupportedStudentTopic = (message) => {
    const normalizedMessage = normalizeText(message);
    ...
    if (matchesAnyTerm(normalizedMessage, tuitionTerms)) {
        return {
            ...
            answer: 'Hiện chatbot AI không hỗ trợ trả lời về học phí, công nợ, biên lai hoặc miễn giảm học phí. Bạn vui lòng kiểm tra kênh thông báo chính thức của nhà trường hoặc gửi yêu cầu hỗ trợ để bộ phận phụ trách kiểm tra.',
        };
    }
    ...
    if (matchesAnyTerm(normalizedMessage, scheduleTerms)) {
        return {
            topic: 'Ngoài phạm vi chatbot',
            answer: 'Hiện chatbot AI không hỗ trợ trả lời về lịch học, lịch thi, thời khóa biểu hoặc đăng ký học phần. Bạn vui lòng kiểm tra kênh thông báo chính thức của nhà trường hoặc gửi yêu cầu hỗ trợ để cán bộ phụ trách kiểm tra.',
        };
    }
    ...
    if (matchesAnyTerm(normalizedMessage, studentConfirmationTerms)) {
        return {
            topic: 'Ngoài phạm vi chatbot',
            answer: 'Hiện chatbot AI không hỗ trợ thủ tục xác nhận sinh viên hoặc các thủ tục hành chính. Bạn vui lòng liên hệ bộ phận phụ trách hoặc gửi yêu cầu hỗ trợ để được hướng dẫn chính thức.',
        };
    }
    return null;
};
const buildStudentSupportPrompt = (message, matchedItem) => {
...
const generateAiAnswer = async (message, matchedItem) => {
...
const knowledgeBase = [
    {
        ...
        answer: 'Bạn có thể điểm danh bằng cách vào mục Điểm danh QR, quét mã do giảng viên hoặc ban cán sự cung cấp và cho phép trình duyệt dùng vị trí nếu hệ thống yêu cầu. Sau khi điểm danh, bạn có thể xem lại tình trạng chuyên cần trong mục Chuyên cần.',
        actions: [
    ...
    },
    {
        ...
        answer: 'Để nộp điểm rèn luyện, bạn vào mục Nộp phiếu DRL, tự đánh giá theo từng tiêu chí và gửi phiếu. Nếu tiêu chí cần minh chứng, hãy vào mục Nộp minh chứng để tải tài liệu lên trước hoặc sau khi kê khai.',
        actions: [
    ...
    },
    {
        ...
        answer: 'Bạn có thể kiểm tra thông tin tài khoản, email và thông tin lớp tại mục Cá nhân. Nếu phát hiện thông tin chưa đúng, hãy gửi yêu cầu hỗ trợ để cán bộ phụ trách kiểm tra và cập nhật.',
    ...
    },
    {
        ...
        answer: 'Bạn có thể theo dõi các hoạt động do khoa hoặc nhà trường tổ chức trong hệ thống. Khi tham gia sự kiện có điểm danh, hãy quét mã QR đúng thời gian quy định để được ghi nhận.',
    ...
    },
    {
        ...
        answer: 'Nếu câu hỏi cần cán bộ kiểm tra trực tiếp, bạn có thể gửi yêu cầu hỗ trợ. Hãy mô tả rõ vấn đề, kèm mã số sinh viên, lớp và ảnh minh chứng nếu có để được xử lý nhanh hơn.',
    },
];
...
export const sendChatbotMessage = async (req, res) => {
    ...
    if (!matchedItem) {
        return res.json({
            answer: 'Mình chưa có dữ liệu chắc chắn cho câu hỏi này. Bạn có thể hỏi về điểm danh, điểm rèn luyện, minh chứng, thông tin cá nhân hoặc gửi yêu cầu hỗ trợ để cán bộ phụ trách xử lý.',
            confidence: 0.25,
